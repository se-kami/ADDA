...
def get_loaders_svhn(data_dir='_DATA', batch_size=32, train_size=0.8, seed=None):
    datasets = get_svhn_data(data_dir, train_size, seed)
    ds_train, ds_dev, ds_test = datasets

    loader_train = DataLoader(ds_train, batch_size=batch_size, shuffle=True)
    loader_dev = DataLoader(ds_train, batch_size=batch_size*2, shuffle=False)
    loader_test = DataLoader(ds_train, batch_size=batch_size*2, shuffle=False)
    return loader_train, loader_dev, loader_test


def get_loaders_mnist(data_dir='_DATA', batch_size=32, train_size=0.8, seed=None):
    # source data
    datasets = get_mnist_data(data_dir, train_size, seed)
    ds_train, ds_dev, ds_test = datasets
...
def get_loaders_svhn_mnist(data_dir='_DATA', batch_size=32, train_size=0.8, seed=None):
    # source data
    ds_src = get_svhn_data(data_dir, train_size, seed)
    ds_src_train, ds_src_dev, ds_src_test = ds_src
    # target data
    ds_trg = get_mnist_data(data_dir, train_size, seed)
    ...
    # dataloaders
    loader_train, loader_src_dev, loader_src_test, loader_trg_dev, loader_trg_test = get_loaders(datasets, batch_size)
